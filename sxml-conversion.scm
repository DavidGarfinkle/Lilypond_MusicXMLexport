			...
			(sxml-match x
				...
				[(music (@ (name ,attribute) (measure (,measure-number #f))) . ,rest) 
					(cond 
						...
						( ; ApplyContext --- produces sxml from current records
...
;; todo return (duration ...) and (type ...) s.t. they are not nested in a list
		...
		(let* (
			(matcher (lambda (lst) (sxml-match lst
				...
				[(duration (@ (log ,log) (dot-count ,dot) (scale ,scale) . ,dur-rest))
					`( 
						(duration ,(log-duration->musicxml-duration-calc log dot))
						(type ,(log-duration->musicxml-duration-names log)))]
	...
	(define (note-event-write sblock measure-number)
		 (measure-list-extend mlist measure-number (note-event-sxml sblock)))
		...
		(let ((note-list (unwrap (cdr (unwrap sblock)))))
			(sxml-match (car note-list)
				...
				(begin
					...
					(measure-list-extend mlist measure-number (map (lambda (lst) (append (lily-note-event->musicxml-note-event lst) '((chord)))) (cdr note-list))))]))) ;for the rest of the notes in the chord, add (chord) as specified in MusicXML



	(search sxml-list)	
	(display (vector->list (measure-list mlist))) (newline)
)
